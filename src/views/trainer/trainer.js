...
const Trainer = () => {
    ...
    const handleEditEntrenador = async () => {
        ...
        if (!response.err) {
            setEntrenadores(entrenadores.map(entrenador =>
                entrenador.id === selectedEntrenador.id ? { ...newEntrenador, id: selectedEntrenador.id } : entrenador
            ));
    ...
    return (
        <CContainer>
            ...
            <CButton style={{ backgroundColor: 'green', color: 'white', marginTop: '15px', marginBottom: '15px' }} onClick={() => setAddEntrenadorModalOpen(true)}>Agregar Entrenador</CButton>
            ...
            <CRow>
                {entrenadores.map((entrenador) => (
                    <CCol sm="6" key={entrenador.id} className="mb-4">
                        <CCard>
                            <CCardHeader>
                                <CCardTitle>{entrenador.EndNombr} {entrenador.EndApell}</CCardTitle>
                            </CCardHeader>
                            <CCardBody>
                                <p>Cédula: {entrenador.EndCedul}</p>
                                <p>Teléfono: {entrenador.EndTelef}</p>
                                <p>Estudios: {entrenador.EndEspec}</p>
                                <p>Subcategoría: {subcategorias.find(sc => sc.id === entrenador.EndIdSubCa)?.name}</p>
                                <CButton style={{ backgroundColor: 'green', color: 'white', marginTop: '15px', marginRight: '10px' }} onClick={() => {
                                    setSelectedEntrenador(entrenador);
                                    setNewEntrenador(entrenador);
                                    setEditEntrenadorModalOpen(true);
                                }}>Editar</CButton>
                                <CButton style={{ backgroundColor: 'red', color: 'white', marginTop: '15px' }} onClick={() => {
                                    setSelectedEntrenador(entrenador);
                                    setModalOpen(true);
                                }}>Eliminar</CButton>
                            </CCardBody>
            ...
            <CModal visible={modalOpen} onClose={() => setModalOpen(false)}>
                ...
                <CModalBody>¿Está seguro de que desea eliminar a {selectedEntrenador?.EndNombr} {selectedEntrenador?.EndApell}?</CModalBody>
                <CModalFooter>
            ...
            <CModal visible={addEntrenadorModalOpen} onClose={() => setAddEntrenadorModalOpen(false)}>
                ...
                <CModalFooter>
                    <CButton style={{ backgroundColor: 'red', color: 'white', marginTop: '15px' }} onClick={() => setAddEntrenadorModalOpen(false)}>Cancelar</CButton>
                    <CButton style={{ backgroundColor: 'green', color: 'white', marginTop: '15px' }} onClick={handleAddEntrenador}>Agregar</CButton>
                </CModalFooter>
            </CModal>
            ...
            <CModal visible={editEntrenadorModalOpen} onClose={() => setEditEntrenadorModalOpen(false)}>
                ...
                <CModalFooter>
                    <CButton style={{ backgroundColor: 'red', color: 'white', marginTop: '15px' }} onClick={() => setEditEntrenadorModalOpen(false)}>Cancelar</CButton>
                    <CButton style={{ backgroundColor: 'green', color: 'white', marginTop: '15px' }} onClick={handleEditEntrenador}>Actualizar</CButton>
                </CModalFooter>
            </CModal>
        </CContainer>
    );
